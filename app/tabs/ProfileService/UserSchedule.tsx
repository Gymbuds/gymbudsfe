...
export default function ScheduleScreen({ navigation }: Props) {
  ...
  const deleteUserTimeRange = async (aval_range_id: any) => {
    try {
      const data = await fetchFunctionWithAuth('avalrange',{
        method: 'DELETE',
  ...
  const handleAddTimeRange = ({ day, start, end }: { day: string; start: string; end: string }) => {
  ...
  const handleStartOpen = (open: any) => {
    if (open) {
      setEndOpen(false);
    }
    setStartOpen(open);
          ...
          <View style={tw`flex-1 justify-center items-center bg-black bg-opacity-50`}>
            ...
            <View style={tw`flex-row items-center mb-4`}>
              ...
              <DropDownPicker
                ...
                onChangeValue={(value) => setStartTimePeriod(value ?? '')}
              />
            </View>
          ...
          <View style={tw`flex-row items-center mb-4`}>
            ...
            {!startOpen && (
              <DropDownPicker
                ...
                onChangeValue={(value) =>setEndTimePeriod(value ?? '')}
              />
            )}
          </View>
